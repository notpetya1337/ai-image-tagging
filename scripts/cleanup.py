                    ...
                    if relpath.endswith(imageextensions):
                        ...
                        if filefound:
                            ...
                            if pic_md5 == docmd5:
                                ...
                                logger.warning(
                                    "MD5 mismatch for image file %s. Local hash is %s, MongoDB hash is %s",
                                    fullpath,
                                    pic_md5,
                                    docmd5,
                                )
                        ...
                        if removepath:
                            logger.info(
                                "Pulling path record, {'_id': %s}, { '$pull': {'relativepath': { '$in': %s}}}",
                                document["_id"],
                                relpath,
                            )
            ...
            for document in list(videocollection.find({"subdiv": subdiv})):
                for relpath in document["relativepath"]:
                    ...
                    if relpath.endswith(videoextensions):
                        ...
                        logger.info("File found status: %s", filefound)
                        if filefound:
                            vid_md5 = get_video_content_md5(
                                os.path.join(rootdir, relpath)
                            )
                            ...
                            if vid_md5 == docmd5:
                                ...
                                logger.warning(
                                    "MD5 mismatch for video file %s. Local hash is %s, MongoDB hash is %s",
                                    fullpath,
                                    vid_md5,
                                    docmd5,
                                )
